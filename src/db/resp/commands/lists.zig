...
pub fn Commands(comptime T: type) type {
    return struct {
        ...
        pub fn lpush(self: *T, key: []const u8, value: []const u8) !u64 {
        ...
        /// Pops value from the left of a list
        pub fn lpop(self: *T, key: []const u8) !?[]const u8 {
        ...
        /// Pushes value to the right of a list
        pub fn rpush(self: *T, key: []const u8, value: []const u8) !u64 {
            if (key.len == 0 or value.len == 0) return RedisError.InvalidArgument;
        ...
        /// Pops value from the right of a list
        pub fn rpop(self: *T, key: []const u8) !?[]const u8 {
            if (key.len == 0) return RedisError.InvalidArgument;
        ...
        pub fn lrange(self: *T, key: []const u8, start: i64, stop: i64) !?[][]const u8 {
            if (key.len == 0) return RedisError.InvalidArgument;
            ...
            const cmd = try self.formatCommand("*4\r\n$6\r\nLRANGE\r\n${d}\r\n{s}\r\n${d}\r\n{s}\r\n${d}\r\n{s}\r\n", .{ key.len, key, start_str.len, start_str, stop_str.len, stop_str });
            defer self.allocator.free(cmd);
