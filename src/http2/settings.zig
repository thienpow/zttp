// src/http2/settings.zig - HTTP/2 settings management
...
pub const Settings = struct {
    ...
    pub fn readPayload(_: Allocator, payload: []const u8) !Settings {
        ...
        while (i < payload.len) : (i += 6) {
            ...
            switch (id) {
                ...
                SettingId.ENABLE_PUSH => settings.enable_push = value,
                SettingId.MAX_CONCURRENT_STREAMS => settings.max_concurrent_streams = value,
                SettingId.INITIAL_WINDOW_SIZE => settings.initial_window_size = value,
                SettingId.MAX_FRAME_SIZE => settings.max_frame_size = value,
    ...
    pub fn writePayload(self: Settings, allocator: Allocator) ![]u8 {
        ...
        const settings = [_]struct { id: u16, value: u32 }{
            ...
            .{ .id = SettingId.ENABLE_PUSH, .value = self.enable_push },
