...
pub fn Commands(comptime T: type) type {
    return struct {
        ...
        pub fn zadd(self: *T, key: []const u8, score: f64, member: []const u8) !u64 {
            ...
            const cmd = try self.formatCommand("*4\r\n$4\r\nZADD\r\n${d}\r\n{s}\r\n${d}\r\n{s}\r\n${d}\r\n{s}\r\n", .{ key.len, key, score_str.len, score_str, member.len, member });
        ...
        pub fn zrange(self: *T, key: []const u8, start: i64, stop: i64) !?[][]const u8 {
        ...
        pub fn zrem(self: *T, key: []const u8, member: []const u8) !u64 {
            if (key.len == 0 or member.len == 0) return RedisError.InvalidArgument;
